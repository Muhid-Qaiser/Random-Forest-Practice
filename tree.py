...
class Node:
    def __init__(self,klasslabel='',pdistribution=[],score=0,wlearner=None, lchild=None, rchild=None):
        """
               Input:
               --------------------------
...
class DecisionTree:
    ...
    def build_tree(self, X, Y, depth):
        ...
        if X.shape[0] == 0:
            return Node(klasslabel=0)
        
        purity = max(counts) / nexamples

        if len(classes) == 1 or depth >= self.maxdepth or purity > 0.95 or nexamples <= self.exthreshold:
            klasslabel = classes[np.argmax(counts)]
            return Node(klasslabel=klasslabel)
        
        # Create a new instance of the weak learner for each node
        wlearner = self.getWeakLearner()
        score, left_indices, right_indices = wlearner.train(X, Y)

        if left_indices is None and right_indices is None or len(left_indices) == 0 or len(right_indices) == 0 :
            klasslabel = classes[np.argmax(counts)]
            return Node(klasslabel=klasslabel)
        ...
        return Node(score=score, lchild=left_subtree, rchild=right_subtree, wlearner=wlearner)
    ...
    def predict(self, X):
        ...
        for idx in range(X.shape[0]):
            ...
            z.append(int(self._predict(self.root,X[idx,:])))
